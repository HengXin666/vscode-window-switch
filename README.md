...
- 高级管理面板只负责窗口排序、分组、重命名和改色：`#颜色 <窗口标题>`。
...
- 点击 Activity Bar 的“同步终端”视图中的窗口后，会打开独立的同步终端页面。该页面只显示所选窗口的多个终端、终端输出和输入区，不再重复绘制窗口栏；终端视图使用 xterm.js 渲染真实终端 PTY 的原始 VT 数据，支持实时光标、颜色、TUI、Zsh 插件输出和滚动，页面宽度变化会同步触发 xterm fit。
  ...
  - 左下角状态栏按钮只打开轻量的快速窗口选择；需要分组、排序和改色时，再从命令面板执行 `Window Deck: 打开高级管理面板`。
- 聚焦能力：
