...
- 插件版本更新后会提示是否重启所有已打开的 VS Code 窗口；确认后每个 Window Deck 窗口会在心跳中收到同一个重启请求并重载自身。
- 聚焦能力：
...
macOS 聚焦窗口依赖 AppleScript 调用 Accessibility API。首次使用如果提示找不到窗口、无法 `AXRaise`、未授权或不允许控制其他应用，请在系统设置中授予当前 VS Code 应用权限：

- `隐私与安全性 > 辅助功能`：允许 `Visual Studio Code`、`Code - Insiders` 或 `VSCodium`。
- `隐私与安全性 > 自动化`：允许 VS Code 控制 `System Events`。
- 如果多个窗口标题相似，先在目标窗口运行 `Window Deck: 应用窗口标题标记`，再重试切换。
