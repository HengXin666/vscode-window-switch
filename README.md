...
- 终端栏目分为两个标签页：
  ...
  - 实时原始数据依赖 VS Code 的 `terminalDataWriteEvent` proposed API。执行 `Window Deck: 安装原生终端同步权限` 后，Window Deck 会请求一次系统授权，把自己加入 VS Code 的 `product.json` API 白名单；Linux 使用 pkexec，macOS 使用系统管理员授权，Windows 使用 UAC 提权。之后正常启动 VS Code 即可，不需要启动参数。VS Code 更新后如权限恢复默认，再执行一次该命令即可。
