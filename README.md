...
- 终端栏目分为两个标签页：
  ...
  - `合并终端` 使用占满编辑器可用区域的“左侧窗口 / 中间终端 / 右侧当前窗口终端”三栏布局，页面本身不会滚动，窗口列表、输出和终端列表分别独立滚动。
  - 中间终端可向当前 VS Code 窗口中选中的原生终端发送命令，并同步展示 Shell Integration 捕获到的命令输出；工具栏终端按钮会聚焦同一个原生终端实例，以使用完整 TUI、快捷键、复制和搜索能力。
- 聚焦能力：
